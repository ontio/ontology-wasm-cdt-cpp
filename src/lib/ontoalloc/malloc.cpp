...
void* memset(void *dst, int32_t value, size_t size);
...
namespace ontology {
   ...
   private:
      ...
      public:
         ...
         {
            //ontology_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
void* malloc(size_t size)
{
   return ontology::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = ontology::memory_heap.malloc(count*size);
...
void* realloc(void* ptr, size_t size)
{
   return ontology::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return ontology::memory_heap.free(ptr);
}
