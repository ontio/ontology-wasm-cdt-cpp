...
namespace ontio {
   ...
   void set_blockchain_parameters(const ontio::blockchain_parameters& params) {
      char buf[sizeof(ontio::blockchain_parameters)];
      ontio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(ontio::blockchain_parameters& params) {
      char buf[sizeof(ontio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      ontio::check( size <= sizeof(buf), "buffer is too small" );
      ontio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            ontio::check(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return ontio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = ontio::memory_heap.malloc(count*size);
...
{
   return ontio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return ontio::memory_heap.free(ptr);
}
