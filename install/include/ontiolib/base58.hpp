...
class crypto_alg {
	public:
	static void crypto_sha256(const std::vector<char> &src , H256 &res) {
	    SHA256((const unsigned char *)src.data(), src.size(), res.data());
	}
	
	static void crypto_hash256(const std::vector<char> &src , H256 &res) {
		H256 t;
		crypto_sha256(src, t);
...
#define sha256 crypto_alg::crypto_sha256
#define hash256 crypto_alg::crypto_hash256
